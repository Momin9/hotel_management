...
class Invoice(models.Model):
    ...
    def calculate_totals(self):
        ...
        # Get tax and service charge rates from tenant settings
        try:
            tenant_settings = TenantSettings.objects.get(tenant__schema_name=connection.tenant.schema_name)
            tax_rate = tenant_settings.tax_rate / 100
            service_rate = tenant_settings.service_charge_rate / 100
        except TenantSettings.DoesNotExist:
            tax_rate = 0.10  # 10% default
            service_rate = 0.05  # 5% default
        
        self.tax_amount = (self.subtotal - self.discount_amount) * tax_rate
        self.service_charge = (self.subtotal - self.discount_amount) * service_rate
